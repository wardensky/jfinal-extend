...
public class AutoTableBindPlugin extends ActiveRecordPlugin {
    ...
    /**
     * @param targetDirName 设置class编译文件夹名称 例如 "target" (只写文件夹名称,不要相对或绝对路径)<br>
     * 默认为 "target"<br>
     * @param excludeTargetDirName 设置忽略class编译文件夹名称 例如 "test-classes" (只写文件夹名称,不要相对或绝对路径)<br>
     * 默认为 "test-classes"<br>
     * @return
     */
    public AutoTableBindPlugin target(String targetDirName, String... excludeTargetDirName) {
    	if (StrKit.notBlank(targetDirName)) {
    		this.targetDirName = targetDirName;
    	}
    	
    	for (String name : excludeTargetDirName) {
    		this.excludeTargetDirName.add(name);
		}
    	return this;
    }
    ...
    public boolean start() {
    	if (excludeTargetDirName.size() == 0) {
			excludeTargetDirName.add(TEST_CLASSES);
		}
    	StringBuilder sb;
    	if (classpath.contains(targetDirName)) {
    		sb = new StringBuilder();
    		//由于unix中file.separator为斜杠"/"，下面这段代码可以处理windows和unix下的所有情况
    		String temp[] = classpath.replaceAll("\\\\","/").split("/");
    		...
    		for (int i = 0; i <= len; i++) {
    			...
    			sb.append(temp[i] + File.separator);
			}
		} else {
			sb = new StringBuilder(classpath);
		}
		List<Class<? extends Model>> modelClasses = ClassSearcher.of(Model.class).jarFilter(this.jarFilter)
				.libDir(this.libDir)
				.classpath(sb.toString())
                .scanPackages(scanPackages).injars(includeJars).includeAllJarsInLib(includeAllJarsInLib).target(targetDirName, excludeTargetDirName).search();
        TableBind tb;
		log.info("modelClasses count = " + modelClasses.size());
        ...
        for (Class modelClass : modelClasses) {
            ...
            //若是抽象类则忽略
            boolean isAbs = Modifier.isAbstract(modelClass.getModifiers());
            if (isAbs) {
				continue;
			}
	...
	public AutoTableBindPlugin jarFilter(String jarFilter) {
